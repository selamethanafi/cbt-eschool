...
if((!empty($key)) and (!empty($url_bank_soal)))
{
	...
	if($cacah_soal == 0)
	{
		$_SESSION['error'] = 'tidak ada soal di server pusat';
...
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
				...
				$stmt->bind_param("iisssssssssss", $id_soal, $nomer_soal, $kode_soal, $pertanyaan, $tipe_soal, $pilihan_1, $pilihan_2, $pilihan_3, $pilihan_4, $pilihan_5, $jawaban_benar, $status_soal, $created_at);
				if ($stmt->execute()) {
						echo "Insert sukses";
						} else {
				...
				$stmt->close();
				$ke++;
				$lanjut = 'unduh_soal_dari_server_pusat.php?kode_soal='.$kode_soal.'&ke='.$ke;
				?>
	...
	else
	{
		...
		$lanjut = 'daftar_butir_soal.php?kode_soal='.$kode_soal;
		?>
		<script>setTimeout(function () {
						   window.location.href= '<?php echo $lanjut;?>';
					},1000);

...
if(empty($jenis))
{
	echo 'Silakan memilih <h1><a href="unduh_jadwal.php?jenis=pht&id=0">PHT</a>  <a href="unduh_jadwal.php?jenis=pas&id=0">PAS</a>  <a href="unduh_jadwal.php?jenis=um&id=0">Asesmen Madrasah</a></h1>';
	die();
}
//echo $key.' '.$url_bank_soal;
			...
			if($json)
			{
				...
				foreach($json as $dm)
				{
					...
					if($pesan == 'ada')
					{
						...
						$sql = "update `soal` set `tanggal` = '$tanggal' where `id_soal` = '$id_soal'";
							//echo $sql;
...
<?php
					$id++;
    					    ...
    					    $lanjut = 'unduh_jadwal.php?jenis='.$jenis.'&id='.$id;
        //					die($lanjut);
                            ?>
